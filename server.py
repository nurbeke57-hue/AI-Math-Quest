...
# Тапсырмалар (мысал ретінде қысқартылған)
tasks = {
...
def train_ai_model():
    """AI үлгісін деректер жиналған соң оқыту"""
    global ai_model
...
def predict_next_level(score, level):
    """Ойыншының келесі деңгейге дайын болу ықтималдығын болжау"""
    if len(training_data) < 5:
        return 0.5
...
def adaptive_difficulty(prob, current_level):
    """Бейімделген ойлау логикасы — үлгерімге қарай деңгейді өзгертеді"""
    if prob > 0.75:
...
def answer(req: AnswerRequest):
    ...
    print(f"[{req.player}] {'✅' if is_correct else '❌'} {req.question} — деңгей {pdata['level']} | Болжау: {prob:.2f}")
...
if __name__ == "__main__":
    ...
    print(f"🌐 Браузерде ашу: {url}")

    # Автоматты түрде браузер ашу (сәл кідірістен кейін)
    time.sleep(1)
    webbrowser.open(url)
